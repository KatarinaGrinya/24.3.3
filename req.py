...
print(f'Имя питомца "{pet_1}" изменено на "{pet_2}"\nКод ответа - 200\n')

print('Запрос данных о питомце с сервера по id - ' + str(pet_id))

# GET-запрос
res_3 = requests.get(f'https://petstore.swagger.io/v2/pet/{pet_id}', headers=headers)

print(res_3.status_code)
print(f'На сервере имеются данные о питомце\nКод ответа - 200\n')

print('Удаление данных о питомце с сервера по id - ' + str(pet_id))

# DELETE-запрос
res_4 = requests.delete(f'https://petstore.swagger.io/v2/pet/{pet_id}', headers=headers)

print(res_4.status_code)
print(f'С сервера удалены данные о питомце\nКод ответа - 200\n')

print('Повторный запрос данных о питомце с сервера по id - ' + str(pet_id))

# Повторный GET-запрос
res_5 = requests.get(f'https://petstore.swagger.io/v2/pet/{pet_id}', headers=headers)

print(res_5.status_code)
print('На сервере нет данных о питомце\nКод ответа - 404')
